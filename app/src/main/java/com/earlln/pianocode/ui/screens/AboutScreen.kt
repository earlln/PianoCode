    ...
    LazyColumn(
        ...
        item {
            InfoCard(title = "악보 코드 변환 사용법") {
                Bullet("악보 이미지는 최근 사진, 갤러리 앱(사진·앨범·스토리), 파일, 카메라 촬영 중에서 가져올 수 있습니다.")
                Bullet("이미지를 고르면 글자 인식으로 코드 심볼을 찾아냅니다.")
                Bullet("원래 조성은 찾아낸 코드로 자동 추정되며, 직접 바꿀 수도 있습니다.")
                Bullet("‘조옮김’은 모든 코드를 같은 간격으로 옮기고, ‘스케일 맞춤’은 도수를 유지한 채 목표 스케일의 화성으로 바꿉니다.")
                Bullet("코드처럼 보이지만 읽지 못한 글자가 있으면 변환 전에 개수를 알려 줍니다. 그 부분은 원래 조성으로 남습니다.")
                Bullet("오선보의 조표와 음표는 바뀌지 않습니다. 코드 심볼만 옮겨 적습니다.")
